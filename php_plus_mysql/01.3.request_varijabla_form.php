...
<html>
    ...
    <body>
         <form method="POST" action="#">
               ...
               Ime: <input required="true" type="text" name="ime" value="<?=(isset($_REQUEST['ime']))?$_REQUEST['ime']:''?>"><Br> 
                Prezime:  <input  required="true" type="text" name="prezime" value="<?=(isset($_REQUEST['prezime']))?$_REQUEST['prezime']:''?>"><Br>
                Spol:<br>
                <!-- (postojispol)? true:false  
                if true (je li spol M)? true:false
                -->
                M<input type="radio" name="spol" value="M" <?=(isset($_REQUEST['spol']))?($_REQUEST['spol']=='M')?'checked':'':''?>><br>
                Ž<input type="radio" name="spol" value="Ž"<?=(isset($_REQUEST['spol']))?($_REQUEST['spol']=='Ž')?'checked':'':''?>><br>
                Županija:<br>
                <input type="color" name="omiljena_boja" value="<?=$_REQUEST['omiljena_boja']?>">
                <select name="zupanija">
                    <?php
                    if(isset($_REQUEST['zupanija'])){
                       echo '<option  selected="true" value="'.$_REQUEST['zupanija'].'">predodabrano</option>'; 
                    }
                    ?>
                ...
                <textarea  required="true" name="napomena" cols="50"><?=$_REQUEST['napomena']?></textarea><br><br>
                <input type="submit" name="potvrdi_btn" value="potvrdi">
            </form>
        
        <?php
                    if(isset($_REQUEST['potvrdi_btn'])){
